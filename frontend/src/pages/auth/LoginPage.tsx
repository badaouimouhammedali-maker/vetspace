...
export function LoginPage() {
  ...
  return (
    <AuthLayout>
      ...
      <p className="mt-1.5 text-body text-ink-muted">{t('login.subtitle')}</p>
      ...
      {superseded ? (
        ...
        >
          <p className="text-sm font-bold text-ink">{t('api.sessionSupersededTitle')}</p>
          <p className="mt-1 text-sm text-ink">{t('api.sessionSuperseded')}</p>
          {/* The one case where the right next step may not be "sign in again". */}
          <p className="mt-2 text-xs text-ink-muted">{t('api.sessionSupersededHint')}</p>
        </div>
      ) : null}

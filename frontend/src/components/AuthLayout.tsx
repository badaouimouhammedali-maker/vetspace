...
export function AuthLayout({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-screen">
      <aside className="on-navy relative hidden w-1/2 flex-col items-center justify-center bg-brand-navy p-12 lg:flex">
        <div className="absolute inset-0 opacity-10 [background:radial-gradient(circle_at_30%_20%,rgb(var(--accent))_0,transparent_45%),radial-gradient(circle_at_75%_70%,rgb(var(--accent))_0,transparent_40%)]" />
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <p className="mt-3 max-w-sm text-body text-on-navy-muted">{t('auth.tagline')}</p>
        </div>
      </aside>
